...
vendor-install:
	ln -svf $(PWD)/../cowpy/src/cowpy $(WORKON_HOME)/$(PACKAGE_NAME)/lib/python3.9/site-packages/

dev-setup: venv vendor-install 

test: SERVER_ROUTE_MODULES := tests.routes
test: vendor-install 
	PYTHONPATH=${PWD}:${PWD}/src SERVER_ROUTE_MODULES=$(SERVER_ROUTE_MODULES) python3 -m $(PACKAGE_NAME).serve 	
...
# -- jroutes will never (probably) need to be available as a system-wide library 
# install:
# ifeq ($(HEAD_TAGGED), 1)
# 	pip install .
# else 
